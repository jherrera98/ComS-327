...
void render_dungeon(dungeon_t *d, monster_t *monsters){
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      ...
      //Checks the positon of each monster
      for (i = 0; i < d->numberOfMonsters; i++)
	{
	  if(p[dim_x] ==  monsters[i].position[dim_x] &&
	     p[dim_y] ==  monsters[i].position[dim_y]) 
	    {
	      char temp = intToChar(monsters[i].characteristic);
	      putchar(temp);
	      monsterPlaced = 1;
	      break;
	    }
      ...
      if(monsterPlaced == 1)
	{
      ...
      else if (p[dim_x] ==  d->pc.position[dim_x] &&
