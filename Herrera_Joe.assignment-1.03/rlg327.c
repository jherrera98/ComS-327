...
int nonTunnelingDistMap[DUNGEON_Y][DUNGEON_X];
...
static void gen_non_tunneling_dist_map(dungeon_t *d)
{
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if(hardnesspair(p->pos) == 0){
      ...
      weight = (hardnesspair(p->pos) / 85) +1;
    }
    ...
    if ((path[p->pos[dim_y]][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]][p->pos[dim_x] - 1].cost > p->cost + weight))
      {
	path[p->pos[dim_y]][p->pos[dim_x] - 1].cost = p->cost + weight;
	heap_decrease_key_no_replace(&h, path[p->pos[dim_y]][p->pos[dim_x] - 1].hn);
      }
    ...
    //Checks the remaining 4 corners
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]-1].hn) && 
	(path[p->pos[dim_y] - 1][p->pos[dim_x]-1].cost > p->cost + weight)) 
      {
	path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost = p->cost + weight;
  ...
  for (y = 1; y < DUNGEON_Y-1; y++) {
	...
	putchar('@');
      ...
      else if (path[y][x].cost == INT_MAX)//Skips empty space
	{
	  putchar(' ');
	}
      else{
...
static void gen_tunneling_dist_map(dungeon_t *d)
{
...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  //Sets the terrain of the character on the map 
  d.map[d.pc[dim_y]][d.pc[dim_x]] = ter_player_char;

  render_dungeon(&d);

  //Prints out the distance maps for the monsters
  gen_non_tunneling_dist_map(&d);
  printf("\n");
  gen_tunneling_dist_map(&d);
