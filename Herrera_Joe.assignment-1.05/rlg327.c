 ...
 if (argc > 1) {
  ...
  while (pc_is_alive(&d) && dungeon_has_npcs(&d) && c != 'q') {
      ...
      while(c != 27 && c != 'q')//breaks out of the while loop when escape is pressed or the q button is pressed
	{
	  ...
	  int tempNumMonsters = 0;
	  i = 0;
	  int j = 0; 
	  for (i = 0; i < 21; i++){
	    for(j = 0; j < 80; j++){
	      if(d.character[i][j] != &d.pc && d.character[i][j] != NULL){
		monsters[tempNumMonsters] = d.character[i][j];
		tempNumMonsters++;
	      }
	    }
	  }
	  ...
	  while(i < d.num_monsters){
	    ...
	    xPosition = d.pc.position[dim_x] - xPosition;// pos if monster is west, Neg if monster is east
	    yPosition = d.pc.position[dim_y] - yPosition;// pos if monster is north, Neg if monster is south
